    ...
    public sealed partial class BBSWindow : Window
    {
        ...
        private void LoadDeviceData()
        {
            ...
            if (loginData != null)
            {
                ...
                {
                    _deviceId36 = loginData.DeviceId;
                    _deviceId = loginData.DeviceId;
                }
                if (!string.IsNullOrWhiteSpace(loginData.DeviceFp))
                {
                    _deviceFp = loginData.DeviceFp;
                }
            }
        }
        ...
        private async void CoreWebView2_WebResourceRequested(CoreWebView2 sender, CoreWebView2WebResourceRequestedEventArgs args)
        {
            ...
            try
            {
                ...
                if (isApiRequest && (uri.Contains("mihoyo.com") || uri.Contains("hoyolab.com")))
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(_deviceFp))
                    {
                        headers.SetHeader("x-rpc-device_fp", _deviceFp);
                    }
                    else if (cookieDic.TryGetValue("DEVICEFP", out var fp) && !string.IsNullOrWhiteSpace(fp))
                    {
        ...
        private JsResult GetHttpRequestHeader()
        {
            ...
            if (!string.IsNullOrWhiteSpace(_deviceFp)) data["x-rpc-device_fp"] = _deviceFp;
            else if (cookieDic.TryGetValue("DEVICEFP", out var fp)) data["x-rpc-device_fp"] = fp;
        ...
        private async Task LoadPageAsync(string url)
        {
            ...
            var loginData = SecureCookieStorage.Get();
            if (loginData != null && !string.IsNullOrWhiteSpace(loginData.CookieString))
            {
                ParseCookie(loginData.CookieString);
                isCookieLoaded = true;
            }
            
            if (!isCookieLoaded && File.Exists(ConfigPath))
            {
                try 
                {
                    var json = await File.ReadAllTextAsync(ConfigPath);
                    var cfg = JsonSerializer.Deserialize<AppConfig>(json, new JsonSerializerOptions { PropertyNameCaseInsensitive = true });
                    if (!string.IsNullOrWhiteSpace(cfg?.Account?.Cookie))
                    {
        ...
        { 
            [JsonPropertyName("cookie_string")] public string CookieString { get; set; } 
