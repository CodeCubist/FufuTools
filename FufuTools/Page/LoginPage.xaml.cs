...
namespace FufuTools
{
    public sealed partial class LoginPage : Page
    {
        ...
        private static readonly JsonSerializerOptions _jsonSaveOptions = new()
        {
            WriteIndented = true,
            TypeInfoResolver = new DefaultJsonTypeInfoResolver()
        };
        ...
        {
            if (IsLoaded && LoginMethodComboBox.SelectedIndex == 1)
            {
                UpdateGameAppIdFromSelection();
                await RestartLoginFlowAsync();
            }
        ...
        /// <summary>
        /// 使用stoken和mid扫web二维码，从而换取完整的Cookie
        /// </summary>
        ...
        private void SaveCredentials(Dictionary<string, string> cookies)
        {
            ...
            var credentials = new LoginDataModel
            {
            ...
            SecureCookieStorage.Save(credentials);
        }
        ...
        private void UpdateStatus(string message, bool isProgress = false, bool closeDialog = false)
        {
            DispatcherQueue.TryEnqueue(async () =>
            {
                ...
                {
                    _statusDialog = new ContentDialog { XamlRoot = this.XamlRoot };
